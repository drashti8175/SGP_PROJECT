(function () {
  ...
  const rules = {
    ...
    confirmPassword: (value, form) => { const pwd = form.querySelector('[name="password"], [id="regPassword"], [id="dPassword"]'); return pwd && value === pwd.value; }
  };
  ...
  function attach(formSelector) {
    ...
    controls.forEach(el => {
      if (!el.dataset.validate) el.dataset.validate = 'enabled';
    ...
    form.addEventListener('submit', function (e) {
      ...
      if (!valid) {
        ...
        if (successMsg) successMsg.textContent = '✅ Form looks good. Submitting...';
      }
    });
